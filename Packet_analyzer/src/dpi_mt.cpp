...
public:
    ...
    bool process(const std::string& input_file, const std::string& output_file) {
        ...
        if (!config_.json_output.empty()) {
            writeJsonReport(config_.json_output);
        }
...
private:
    ...
    void writeJsonReport(const std::string& filepath) {
        std::ofstream jout(filepath);
        if (!jout.is_open()) {
            std::cerr << "Cannot open json output file: " << filepath << "\n";
            return;
        }
        ...
        jout << "{\n";
        jout << "  \"summary\": {\n";
        jout << "    \"total_packets\": " << stats_.total_packets.load() << ",\n";
        jout << "    \"total_bytes\": " << stats_.total_bytes.load() << ",\n";
        jout << "    \"tcp_packets\": " << stats_.tcp_packets.load() << ",\n";
        jout << "    \"udp_packets\": " << stats_.udp_packets.load() << ",\n";
        jout << "    \"forwarded\": " << stats_.forwarded.load() << ",\n";
        jout << "    \"dropped\": " << stats_.dropped.load() << "\n";
        jout << "  },\n";

        jout << "  \"thread_stats\": {\n";
        jout << "    \"lbs\": [\n";
        for (size_t i = 0; i < lbs_.size(); i++) {
            jout << "      {\"id\": " << i << ", \"dispatched\": " << lbs_[i]->dispatched() << "}"
                 << (i + 1 < lbs_.size() ? "," : "") << "\n";
        }
        jout << "    ],\n";
        jout << "    \"fps\": [\n";
        for (size_t i = 0; i < fps_.size(); i++) {
            jout << "      {\"id\": " << i << ", \"processed\": " << fps_[i]->processed() << "}"
                 << (i + 1 < fps_.size() ? "," : "") << "\n";
        }
        jout << "    ]\n";
        jout << "  },\n";
        ...
        for (size_t i = 0; i < sorted_apps.size(); i++) {
            ...
            double pct = total > 0 ? (100.0 * count / total) : 0;
            jout << "    {\n";
            jout << "      \"name\": \"" << appTypeToString(app) << "\",\n";
            jout << "      \"count\": " << count << ",\n";
            jout << "      \"percentage\": " << std::fixed << std::setprecision(2) << pct << "\n";
            jout << "    }" << (i + 1 < sorted_apps.size() ? "," : "") << "\n";
        }
        jout << "  ],\n";
        ...
        size_t sni_idx = 0;
        for (const auto& [sni, app] : stats_.detected_snis) {
            jout << "    {\"sni\": \"" << sni << "\", \"app\": \"" << appTypeToString(app) << "\"}"
                 << (sni_idx + 1 < stats_.detected_snis.size() ? "," : "") << "\n";
            sni_idx++;
        }
        jout << "  ]\n";
        jout << "}\n";
...
Options:
  ...
  --json-output <file>   Write structured telemetry JSON report
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 3; i < argc; i++) {
        ...
        else if ((arg == "--json-output" || arg == "--json") && i + 1 < argc) cfg.json_output = argv[++i];
